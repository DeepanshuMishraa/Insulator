...
impl Insulator {
    ...
    fn spawn_commit_message_generation(
        ...
        cx.spawn(async move |insulator, cx| {
            ...
            let result = cx
                ...
                .spawn(async move {
                    match workspace_client.request(
                        insulator_client::WorkspaceOperation::GenerateCommitMessage {
        ...
        cx.spawn(async move |insulator, cx| {
            ...
            let result = cx
                ...
                .spawn(async move {
                    let operation = match action {
                        CommitAction::Commit => insulator_client::WorkspaceOperation::Commit {
                    ...
                    let result = match workspace_client.request(operation) {
                        Ok(insulator_client::WorkspaceResult::Ack) => Ok(()),
            ...
            let focus = insulator.update(cx, |insulator, cx| {
                ...
                let focus = match result {
                    Ok(()) => {
                        let dialog_was_open = insulator
                            .commit_dialog
                            .as_ref()
                            .is_some_and(|dialog| dialog.id == id);
                        if dialog_was_open {
                            insulator.commit_dialog = None;
                        }
                        insulator.show_success_toast(match action {
    ...
    pub(super) fn render_commit_dialog(&mut self, cx: &mut Context<Self>) -> Option<AnyElement> {
        ...
        let layer = div()
            ...
            .on_mouse_down(
                MouseButton::Left,
                cx.listener(|insulator, _, window, cx| insulator.close_commit_dialog(window, cx)),
            )
            .child(card);
    ...
    div()
        ...
        .when(enabled, |row| {
            row.on_click(move |_, window, cx| {
                let _ = click_weak.update(cx, |insulator, cx| {
                    insulator.request_commit_action(action, window, cx)
                });
            })
            .on_key_down(move |event: &KeyDownEvent, window, cx| {
                if !event.keystroke.modifiers.modified()
                    && matches!(event.keystroke.key.as_str(), "enter" | "space")
                {
                    let _ = key_weak.update(cx, |insulator, cx| {
                        insulator.request_commit_action(action, window, cx)
                    });
                    cx.stop_propagation();
                }
