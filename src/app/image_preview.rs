...
actions!(insulator_image_preview, [DismissImagePreview]);
...
impl Insulator {
    ...
    pub(super) fn image_for_reference(
        ...
        let attachment_reference =
            reference.starts_with(insulator_protocol::attachments::ATTACHMENT_SCHEME);
        ...
        cx.spawn(async move |insulator, cx| {
            let image = cx
                .background_executor()
                .spawn(async move {
                    insulator_client::persistence::read_remote_reference(
                        &fetch_reference,
