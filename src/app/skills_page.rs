...
//! ([`Insulator::ensure_skills_catalog`]); frames read only the cached catalog.
...
/// in [`Insulator::sync_skills_rows`]: a changed row — catalog identity, enabled
            ...
            let catalog = cx
                ...
                .spawn(async move {
                    match daemon.request(
                        Uuid::nil(),
                        Uuid::nil(),
                        insulator_client::Command::LoadSkills { projects },
                    )? {
                        insulator_client::ResponsePayload::SkillsCatalog { catalog } => Ok(catalog),
            ...
            let result = cx
                ...
                .spawn(async move {
                    daemon.request(
                        Uuid::nil(),
                        Uuid::nil(),
                        insulator_client::Command::SetSkillsEnabled { dirs, enabled },
                    )
                })
                .await;
            let _ = this.update(cx, |this, cx| {
            ...
            let result = cx
                ...
                .spawn(async move {
                    daemon.request(
                        Uuid::nil(),
                        Uuid::nil(),
                        insulator_client::Command::TrashSkills { dirs },
                    )
                })
                .await;
            let _ = this.update(cx, |this, cx| {
                ...
                div()
                    ...
                    .when(!selected, |element| {
                        element.hover({
                            ...
                            // stationary cursor and the highlight would
                            // strobe across them trying to keep up. Hide it
                            // until the list is still again; rows rebuild
                            // every scroll frame, so the gate is consulted
                            // fresh, and the gate's wake repaints once it
                            // reopens.
                            let hover_gate = self.skills_hover_gate.clone();
