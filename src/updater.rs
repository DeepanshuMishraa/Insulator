...
//! itself with a production build. `INSULATOR_PREVIEW_UPDATE=1` fakes only the
//! automatic sidebar result while retaining the real Sparkle flow for the
//! Check for Updates menu; `INSULATOR_FORCE_UPDATER=1` exercises everything for
...
/// The compact state rendered by Insulator. Update details remain owned by
...
mod macos {
    ...
    struct UserDriverIvars {
        ...
        /// use Sparkle's own windows. Scheduled checks stay inside Insulator.
    ...
    define_class!(
        ...
        #[name = "InsulatorSparkleUserDriver"]
    ...
    impl Updater {
        ...
        pub fn init() -> Option<Self> {
            ...
            if handle.is_null() {
                ...
                eprintln!("Insulator updater: failed to load Sparkle: {reason}");
                return None;
            }
            ...
            if !started {
                eprintln!("Insulator updater: Sparkle rejected its updater configuration");
                return None;
            }
    ...
    /// (Contents/MacOS/Insulator → Contents/Frameworks/Sparkle.framework/Sparkle).
    ...
    mod tests {
        ...
        fn routing_user_driver_satisfies_sparkle_protocols() {
            ...
            let library = target_dir
                .join("debug/Insulator Debug.app/Contents/Frameworks/Sparkle.framework/Sparkle");
            if !library.exists() {
                return;
            }
...
mod feed {
    ...
    /// Compare dotted release numbers field by field. Insulator's versions are
...
<rss xmlns:sparkle="http://www.andymatuschak.org/xml-namespaces/sparkle" version="2.0">
  <channel>
    <item>
      ...
      <enclosure url="https://releases.insulator.sh/Insulator-0.1.4-x86_64-Setup.exe" length="1024" type="application/octet-stream" sparkle:edSignature="oldsig" />
    </item>
    <item>
      <title>0.2.0</title>
      <sparkle:shortVersionString>0.2.0</sparkle:shortVersionString>
      <enclosure url="https://releases.insulator.sh/Insulator-0.2.0-x86_64-Setup.exe" length="2048" type="application/octet-stream" sparkle:edSignature="newsig" />
    </item>
  </channel>
</rss>"#;
        ...
        fn the_newest_signed_item_wins_regardless_of_feed_order() {
            ...
            assert!(item.url.ends_with("Insulator-0.2.0-x86_64-Setup.exe"));
        }
...
mod windows {
    ...
    const PUBLIC_ED_KEY: &str = env!("INSULATOR_SPARKLE_PUBLIC_ED_KEY");
    ...
    impl Updater {
        pub fn init() -> Option<Self> {
            ...
            if verifying_key().is_none() {
                eprintln!("Insulator updater: SUPublicEDKey is not a valid ed25519 key");
                return None;
            }
        ...
        fn start_check(&self, user_initiated: bool) {
            ...
            let spawned = std::thread::Builder::new()
                .name("insulator-updater-check".into())
                .spawn(move || {
        ...
        pub fn set_automatically_checks_for_updates(&self, enabled: bool) {
            ...
            let _ = std::thread::Builder::new()
                .name("insulator-updater-preference".into())
    ...
    fn download_and_verify(item: &AppcastItem) -> anyhow::Result<PathBuf> {
        ...
        let installer = directory.join("Insulator-Setup.exe");
    ...
    mod tests {
        ...
        fn a_signature_from_the_release_script_verifies_here() {
            ...
            const PAYLOAD: &[u8] = b"Insulator-0.0.0-x86_64-Setup.exe contents";
