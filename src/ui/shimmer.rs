...
pub fn loading_animation(duration: Duration, once: bool) -> Animation {
    if once {
...
impl RenderOnce for ShimmerText {
    fn render(self, _: &mut Window, cx: &mut App) -> impl IntoElement {
        ...
        let container = self.base.min_w_0();
...
fn shimmer_highlight_color(
    ...
    let highlight = override_color.unwrap_or_else(|| {
        if dark {
            Hsla {
                l: (text.l + 0.35).min(1.0),
                ..text
            }
        } else {
            Hsla {
                l: (text.l - 0.35).max(0.0),
                ..text
            }
        }
    });
...
mod tests {
    ...
    fn test_shimmer_builder() {
        ...
        let mut text = ShimmerText::new("Thinking")
            .id("thinking")
        ...
        assert_eq!(text.base.style().opacity, Some(0.8));
