...
const help = `Build, notarize, and publish a production release of Insulator.
...
https://releases.insulator.sh. One-time setup lives in RELEASING.md.

Options:
  --local                       Build, notarize, and write the DMG + zip
                                without publishing to R2
  --force                       Publish even if this version is already in R2
  --output <path>               DMG output path (default: dist/Insulator-<version>.dmg)
  --signing-identity <name>     Developer ID Application identity selector
                                (or INSULATOR_SIGNING_IDENTITY; required unless --adhoc)
  --notary-profile <name>       notarytool keychain profile
                                (default: NOTARY; or INSULATOR_NOTARY_PROFILE)
  --build-number <number>       CFBundleVersion override (or INSULATOR_BUILD_NUMBER;
                                default derives a monotonic number from the
                                Cargo version)
  --volume-name <name>          Mounted DMG name (default: Insulator)
  --skip-build                  Reuse target/release/insulator, insulator_js_repl, and
                                insulator-daemon
...
Environment:
  INSULATOR_SIGNING_IDENTITY         Developer ID Application identity selector
  INSULATOR_R2_REMOTE                rclone remote name (default: r2)
  INSULATOR_R2_BUCKET                R2 bucket name (default: insulator-releases)
  INSULATOR_DOWNLOAD_URL_PREFIX      base URL served by the bucket
                                (default: ${defaultDownloadUrlPrefix})
  INSULATOR_HISTORY_COUNT            prior archives pulled for deltas (default: 15)
  INSULATOR_NO_HISTORY=1             skip pulling prior archives (no deltas)
  SPARKLE_BIN                   Sparkle tools dir (default: the bundle.sh cache
                                under .insulator-cache/sparkle)
...
const configuredSigningIdentity =
  values["signing-identity"] ?? process.env.INSULATOR_SIGNING_IDENTITY;
const notaryProfile =
  values["notary-profile"] ??
  process.env.INSULATOR_NOTARY_PROFILE ??
  defaultNotaryProfile;
const explicitBuildNumber =
  values["build-number"] ?? process.env.INSULATOR_BUILD_NUMBER;
...
const r2Remote = process.env.INSULATOR_R2_REMOTE ?? "r2";
const r2Bucket = process.env.INSULATOR_R2_BUCKET ?? "insulator-releases";
...
const downloadUrlPrefix =
  process.env.INSULATOR_DOWNLOAD_URL_PREFIX ?? defaultDownloadUrlPrefix;
const historyCount = Number(process.env.INSULATOR_HISTORY_COUNT ?? "15");
const skipHistory = process.env.INSULATOR_NO_HISTORY === "1";
...
if (!adhoc && !configuredSigningIdentity) {
  throw new Error(
    "Set INSULATOR_SIGNING_IDENTITY or pass --signing-identity (or use --adhoc).",
  );
}
...
if (!Number.isSafeInteger(historyCount) || historyCount < 0) {
  throw new Error("INSULATOR_HISTORY_COUNT must be a non-negative integer.");
}
    ...
    if (detail.includes("directory not found")) {
      throw new Error(
        `R2 bucket "${r2Bucket}" does not exist on remote "${r2Remote}". ` +
          ...
          "releases.insulator.sh custom domain (see RELEASING.md), then re-run.",
      );
    }
  ...
  const requests = [
    {
      ...
      params: {
        ...
        capabilities: {},
        clientInfo: { name: "insulator-release", version: "1" },
      },
    },
...
try {
  ...
  await $`env INSULATOR_CODESIGN_IDENTITY=${identity} INSULATOR_SKIP_CARGO_BUILD=${values["skip-build"] ? "1" : "0"} ${join(projectRoot, "scripts", "bundle.sh")} release`;
  ...
  temporaryDirectory = await mkdtemp(join(tmpdir(), "insulator-dmg-"));
  ...
  // archive as Insulator-<version>.md; generate_appcast links it as the update's
  ...
  if (publishing) {
    ...
    console.log(`\nInsulator ${version} (build ${buildNumber}) is live:`);
