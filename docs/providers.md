...
How Insulator talks to each coding agent: the process it launches, the wire protocol
...
polled off disk, and the one Insulator generates itself — is in
[titles.md](titles.md).
...
| Amp streaming-JSON session (NDJSON over stdio) | [driver/amp.rs](../crates/insulator-core/src/driver/amp.rs) | Amp |
| Harness client API (typed HTTP + downlink streams) | [driver/deepseek.rs](../crates/insulator-core/src/driver/deepseek.rs) | DeepSeek Harness |
...
Inputs ([driver/mod.rs:67](../crates/insulator-core/src/driver/mod.rs#L79)):
...
Outputs ([model.rs:973](../crates/insulator-core/src/model.rs)): `Connected`,
...
[driver/activity.rs](../crates/insulator-core/src/driver/activity.rs), so the transcript renders
...
`Insulator::runtimes` keyed by session id. Runtimes are per session, not per view:
...
([opencode_session.rs](../crates/insulator-core/src/opencode_session.rs)). Insulator quitting without
...
([driver/codex.rs:164](../crates/insulator-core/src/driver/codex.rs#L164)), plus `-c` config
overrides when Computer Use is on.

**Protocol** — newline-delimited JSON-RPC over stdio, genuinely bidirectional:
Codex can send Insulator requests (approvals) and Insulator answers them by id. Three
...
2. `initialized`.
3. `skills/extraRoots/set` when Computer Use is on, so Insulator's bundled skill is
...
**Inbound stream** ([driver/codex.rs:851](../crates/insulator-core/src/driver/codex.rs#L882)):
...
per-peer, the reader only treats method-less messages as replies to Insulator's own
requests ([driver/codex.rs:779](../crates/insulator-core/src/driver/codex.rs#L809)).
...
([driver/codex.rs:660](../crates/insulator-core/src/driver/codex.rs#L690)).

**Models** — a throwaway app-server, `model/list` paged via `nextCursor`, up to
32 pages ([model_catalog.rs:367](../crates/insulator-core/src/model_catalog.rs#L367)).

**Computer Use** — `-c mcp_servers.insulator_js_repl.command=…` registers Insulator's
QuickJS MCP server, with several `-c` flags disabling Codex's own external
computer-use plugin/MCP/skill so only Insulator's `js` / `js_reset` surface is
visible.
...
([pi.rs:39](../crates/insulator-core/src/driver/pi.rs#L39)) carries every divergence,
...
| Whole-session copy | in place | only at launch, so Insulator shells out (see below) |
| Computer Use | Insulator's Pi extension | none — Oh My Pi ships its own `/computer` |
...
`omp --mode rpc --yolo`
([pi.rs:246](../crates/insulator-core/src/driver/pi.rs#L246)). Oh My Pi negotiates
...
Insulator stamps each request with a string id (`insulator-<n>`) and Pi answers with
...
with a 10 s timeout ([pi.rs:800](../crates/insulator-core/src/driver/pi.rs#L800));
...
**Inbound stream** ([pi.rs:1182](../crates/insulator-core/src/driver/pi.rs#L1182)):
...
access only" ([pi.rs:209](../crates/insulator-core/src/driver/pi.rs#L209)).
Pi has no permission system at all, so `--approve` is the whole story. Oh My Pi
*does* have one, which Insulator's `--yolo` then bypasses — the restriction is Insulator's
...
([pi.rs:996](../crates/insulator-core/src/driver/pi.rs#L996)). Rewind adopts the fork
...
performs in place. Oh My Pi only copies at launch, so Insulator shells out to a
throwaway `omp --mode rpc --yolo --fork <session file>` and reads the new cursor
off it ([pi.rs:1108](../crates/insulator-core/src/driver/pi.rs#L1108)). That is the
...
([model_catalog.rs](../crates/insulator-core/src/model_catalog.rs)).

**Computer Use** — Pi only: `--extension <insulator pi extension>` and
`--skill <SKILL.md>`, with the REPL and helper paths passed through the
environment. Insulator's bridge is written against Pi's extension API, and Oh My Pi
...
([driver/claude.rs](../crates/insulator-core/src/driver/claude.rs)), plus `--model`, `--effort`,
...
| `user` with `isReplay: true` | ignored — Insulator's own prompt echoed by `--replay-user-messages` |
| `result` | `TurnFinished` |
...
`permission_suggestions`. Insulator answers with a `control_response` whose result is
...
**Native checkpoints** — after each turn Insulator reads Claude's own transcript at
...
`provider_resume_at` ([claude_session.rs](../crates/insulator-core/src/claude_session.rs)). That
...
with fresh uuids; the returned id map is applied to Insulator's retained turns.
...
routes resolved through CC Switch. Insulator probes it in the background and caches
the last successful catalog; the curated list is only the startup/failure
fallback ([model_catalog.rs](../crates/insulator-core/src/model_catalog.rs)).
...
([driver/amp.rs](../crates/insulator-core/src/driver/amp.rs)). `--stream-json-thinking` implies
...
permission request on its stream; its rules live in `amp permissions`, so Insulator
...
**Branch** — `amp threads export <id>` dumps the thread, Insulator keeps the retained
...
(`INSULATOR_AMP_BRANCH_CONTEXT_V1`). Forking a thread that was itself seeded this way
re-expands the nested envelope first, so branches of branches stay flat
([amp_session.rs](../crates/insulator-core/src/amp_session.rs)).
...
([driver/opencode.rs](../crates/insulator-core/src/driver/opencode.rs)). Insulator already started this
...
only under `/api`, and since this server is Insulator's alone, filtering by
...
([opencode_session.rs](../crates/insulator-core/src/opencode_session.rs)).
...
([driver/acp.rs](../crates/insulator-core/src/driver/acp.rs)).
...
to. Alongside Codex's app-server, this is the only transport where Insulator's
...
capability, since Insulator does not proxy the agent's file or terminal access — an
...
`session/resume` return provider-owned `configOptions`. Insulator resolves the CLI's
...
an older Cursor agent advertises no model option, Insulator retains the legacy
...
Insulator first selects Fx's `gateway` provider option and reads the refreshed model
option from that response. Insulator then targets the exact `model` id with
...
[kimi_session.rs](../crates/insulator-core/src/kimi_session.rs) reads it, and
...
| `user_message_chunk` | ignored — Insulator's own prompt echoed back |
...
than assistant content, so Insulator filters that prelude from the transcript.
...
whole basis for the user's decision. Outside Supervised, Insulator answers for the
...
services *Insulator offers the agent*, not permissions the agent needs. `fs` exists so
an editor can serve unsaved buffer contents in place of what is on disk, and
`terminal` lets the agent run commands through the client's own terminal. Insulator
...
Advertising a capability Insulator cannot service is the harmful choice, because the
...
The one case that would justify serving `fs/read_text_file` is Insulator's own file
...
**Access modes** — Fx exposes native `ask` and `code` modes, so Insulator maps
Supervised to `ask` and the auto modes to `code`. Every other ACP agent stays in
its ordinary execution mode, and `auto_approve` decides whether Insulator answers
...
name**, and the two disagree: Insulator sends `mode` by default, but Kimi's `mode` is
...
advertise steering. Follow-ups remain in Insulator's queue and start after the
...
([grok_session.rs](../crates/insulator-core/src/grok_session.rs)), Cursor re-seeds a
fresh session ([cursor_session.rs](../crates/insulator-core/src/cursor_session.rs)).
...
Insulator's `RuntimeMode` (Supervised / Auto-accept edits / Auto / Full access)
maps into each CLI's own vocabulary.

| Insulator | Codex (`approvalPolicy` / `sandbox` / reviewer) | Claude `--permission-mode` | Cursor | Fx | OpenCode | Grok | Kimi Code |
| --- | --- | --- | --- | --- | --- | --- | --- |
...
persisted with the session and is what makes a Insulator task outlive its process:
...
| Provider | T3 Code transport | Insulator transport |
| --- | --- | --- |
...
streaming-input protocol, which Insulator speaks directly. No Node sidecar and no npm
dependency.

Insulator goes one further than the comparison: Amp and Pi, which T3 Code does not
support, are long-lived here too. Every provider holds a session.

What the long-lived session buys, and what Insulator pays for not having it:

| Capability | T3 Code | Insulator |
| --- | --- | --- |
...
`capabilities` record. Insulator's equivalent surface is split between
...
Note the parts that are *not* a gap. Insulator's Codex path is the same app-server
...
1. Add the variant to `ProviderKind`
   ([model.rs](../crates/insulator-protocol/src/model.rs)) with `id`,
...
3. Pick a transport, and look hard before settling for the one-shot path. Ask
   whether the CLI speaks ACP (`acp` / `agent stdio` — [driver/acp.rs](../crates/insulator-core/src/driver/acp.rs)
