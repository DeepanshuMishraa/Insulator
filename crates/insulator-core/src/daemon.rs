...
fn migrate_projectless_state(
    ...
    for index in indices {
        ...
        let workspace = crate::projectless::migrate_workspace(&old_path).with_context(|| {
            format!(
                "could not move projectless workspace {} under ~/.insulator/projects",
                old_path.display()
            )
        })?;
...
fn event_to_wire(event: DriverEvent) -> anyhow::Result<WireDriverEvent> {
    let (kind, payload) = match event {
        ...
        DriverEvent::PlanModeSwitchFailed(message) => {
            ("planModeSwitchFailed", Value::String(message))
        }
...
pub fn event_from_wire(event: WireDriverEvent) -> anyhow::Result<DriverEvent> {
    ...
    Ok(match event.kind.as_str() {
        ...
        "planModeSwitchFailed" => {
            DriverEvent::PlanModeSwitchFailed(serde_json::from_value(payload)?)
        }
        "extensionStatus" => {
