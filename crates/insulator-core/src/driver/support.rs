...
fn build_opencode_computer_use_config(
    ...
    mcp.insert(
        "insulator_js_repl".into(),
        serde_json::json!({
            "type": "local",
            "command": [repl_path.display().to_string()],
            "enabled": true,
            "environment": {
                "INSULATOR_COMPUTER_USE_SERVER": server_path.display().to_string(),
                "INSULATOR_COMPUTER_USE_PROCESS_DIRECTORY": process_directory.display().to_string(),
            },
        }),
    );
...
pub(super) fn opencode_computer_use_environment(
    ...
    vec![
        ...
        (
            "INSULATOR_COMPUTER_USE_SERVER".to_owned(),
            base.server_path.display().to_string(),
        ),
        (
            "INSULATOR_COMPUTER_USE_PROCESS_DIRECTORY".to_owned(),
...
fn build_grok_computer_use_config(
    ...
    let rules = fs::read_to_string(&base.skill_path).with_context(|| {
        format!(
            "could not read Insulator Computer Use skill {}",
...
fn build_grok_computer_use_toml(
    ...
    environment.insert(
        "INSULATOR_COMPUTER_USE_SERVER".into(),
        toml::Value::String(base.server_path.display().to_string()),
    );
    environment.insert(
        "INSULATOR_COMPUTER_USE_PROCESS_DIRECTORY".into(),
    ...
    mcp_servers.insert("insulator_js_repl".into(), toml::Value::Table(server));
...
/// a GLOBAL store shared with the user's own terminal, so a Full Access Insulator
...
mod tests {
    ...
    fn opencode_computer_use_config_preserves_existing_inline_config() {
        let content = build_opencode_computer_use_config(
            ...
            Path::new("/Applications/Insulator Computer Use"),
            Path::new("/Applications/Insulator.app/Contents/Resources/insulator_js_repl"),
            Path::new(
                "/Applications/Insulator.app/Contents/Resources/skills/insulator-computer-use/SKILL.md",
            ),
            Path::new("/tmp/insulator computer use/session"),
        )
        .unwrap();
        ...
        assert_eq!(
            value
                .pointer("/mcp/insulator_js_repl/command/0")
                .and_then(Value::as_str),
            Some("/Applications/Insulator.app/Contents/Resources/insulator_js_repl")
        );
        assert_eq!(
            value
                .pointer("/mcp/insulator_js_repl/environment/INSULATOR_COMPUTER_USE_SERVER")
                .and_then(Value::as_str),
            Some("/Applications/Insulator Computer Use")
        );
        assert_eq!(
        ...
        assert!(value.pointer("/mcp/insulator_computer_use").is_none());
    }

    #[test]
    fn grok_computer_use_config_preserves_existing_config_and_replaces_insulator_server() {
        ...
        let server = value
            .get("mcp_servers")
            .and_then(|mcp| mcp.get("insulator_js_repl"))
            .unwrap();
        assert_eq!(
        ...
        assert_eq!(
            server
                .get("env")
                .and_then(|env| env.get("INSULATOR_COMPUTER_USE_SERVER"))
                .and_then(toml::Value::as_str),
            Some("/tmp/Insulator Computer Use")
        );
    }
    ...
    fn grok_computer_use_command_is_process_scoped_and_loads_rules() {
        let config = HeadlessComputerUseConfig::Grok {
            ...
            grok_home: PathBuf::from("/tmp/insulator-computer-use/session/grok-home"),
            auth_path: Some(PathBuf::from("/Users/test/.grok/auth.json")),
            rules: "Insulator Computer Use rules".into(),
        };
        let (arguments, environment) = grok_computer_use_launch_configuration(Some(&config));
        assert_eq!(arguments, ["--rules=Insulator Computer Use rules"]);
        let environment = environment.into_iter().collect::<HashMap<_, _>>();
        assert_eq!(
            environment.get("GROK_HOME"),
            Some(&"/tmp/insulator-computer-use/session/grok-home".into())
        );
        assert_eq!(
    ...
    fn provider_stderr_keeps_cli_argument_errors_compact() {
        let message = provider_stderr_error(vec![
            ...
            "name: insulator-computer-use".into(),
