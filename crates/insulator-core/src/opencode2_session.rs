...
//!   export yields a transcript of nothing but placeholders. Insulator exports
...
/// The service caps nothing, so the page size is Insulator's choice. Large enough
...
/// chooses to redact cannot leak anything into Insulator.
...
/// Projects stored messages onto Insulator's turn model.
///
...
/// while a turn is running, whereas Insulator's contract here is "drop the last N
...
fn fork_boundary(
    ...
    if retained_turns > message_ids.len() {
        bail!(
            "OpenCode 2 has only {} native turns, but Insulator needs {retained_turns}",
            message_ids.len()
        );
    }
...
fn retained_turn_count(total_turns: usize, turns_to_remove: usize) -> anyhow::Result<usize> {
    total_turns.checked_sub(turns_to_remove).ok_or_else(|| {
        anyhow!(
            "OpenCode 2 has only {total_turns} native turns, but Insulator needs to remove {turns_to_remove}"
        )
    })
}
...
mod tests {
    ...
    fn summaries_keep_the_stored_directory_and_convert_millis_to_seconds() {
        let project_dir = std::env::temp_dir().join("insulator-opencode2-project");
        let untitled_dir = std::env::temp_dir().join("insulator-opencode2-untitled");
    ...
    fn a_missing_service_lists_nothing_and_starts_nothing() {
        let binary = std::env::temp_dir().join("insulator-opencode2-absent");
    ...
    #[test]
    #[ignore = "requires a running opencode2 background service and INSULATOR_OPENCODE2_TEST_SESSION_ID"]
    fn imports_a_real_transcript_without_redaction_placeholders() {
        let binary =
            crate::command_env::find_executable("opencode2").expect("opencode2 is not installed");
        let session_id = std::env::var("INSULATOR_OPENCODE2_TEST_SESSION_ID")
            .expect("set INSULATOR_OPENCODE2_TEST_SESSION_ID to a completed session");
        let history =
