...
use insulator_protocol::{DAEMON_TOKEN_ENV, DaemonReady, PROTOCOL_VERSION};
...
fn main() -> anyhow::Result<()> {
    ...
    let task_path = insulator_core::persistence::StateStore::default_path();
    let settings = insulator_core::DaemonSettingsStore::open_with_legacy(
        insulator_core::DaemonSettings::default_path(),
        [task_path.with_file_name("settings.json")],
    )
    .context("could not load daemon settings")?;
    let task_store = insulator_core::persistence::StateStore::daemon(task_path);
...
mod tests {
    ...
    fn parses_repeated_browser_origin_allowlist_entries() {
        ...
        assert_eq!(
            ...
            ["https://app.insulator.test", "http://localhost:3000"]
        );
