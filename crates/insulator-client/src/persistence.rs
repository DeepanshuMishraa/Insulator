...
use insulator_protocol::computer_use::ComputerAppGrant;
use insulator_protocol::i18n::AppLanguage;
use insulator_protocol::identity::DATA_DIRECTORY_NAME;
pub use insulator_protocol::model::{
...
pub struct AppSettings {
    ...
    /// UI typeface used by the application chrome and prose.
    ...
    fn migrate_loaded(&mut self) {
        for session in &mut self.sessions {
            let checkpoint_totals_current = session.turns.iter().all(|turn| {
                turn.checkpoint
                    .as_ref()
                    .is_none_or(insulator_protocol::model::Checkpoint::totals_are_current)
            });
            let before = (
                session.turns.len(),
    ...
    pub fn load(&self) -> io::Result<PersistedState> {
            ...
            .map_err(to_io_error)?
        {
            ResponsePayload::TaskState {
                ...
                insulator_protocol::projectless::set_workspace_root(projectless_root);
...
pub fn read_remote_reference(
    ...
    let command = if insulator_protocol::blob::is_reference(reference) {
        Command::ReadBlob {
            reference: reference.to_owned(),
        }
    } else if reference.starts_with(insulator_protocol::attachments::ATTACHMENT_SCHEME) {
